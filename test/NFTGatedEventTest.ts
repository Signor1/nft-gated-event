...
describe("NFT Gated Event", function () {
  ...
  describe("NFT Gated Event Contract", function () {
    ...
    it("It should revert if user doesn't have NFT during registration", async function () {
      ...
      await expect(
        event.connect(anotherUser).registerForEvent(1)
      ).to.be.rejectedWith("not eligible for event");
    });

    it("Should allow users with NFT register for event", async function () {
      const { eventNFT, event, owner, anotherUser } = await loadFixture(
        deployEventContract
      );
      ...
      expect(
        await event.checkRegistrationValidity(1, anotherUser.address)
      ).to.be.equal(true);
